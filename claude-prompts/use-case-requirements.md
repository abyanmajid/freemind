...
Your task is to thoroughly analyse the FreeMind codebase and generate a comprehensive Use Case Requirements document organised around concrete, specific improvements to the application.

Start by reading the UX audit report at claude-prompts/ux-audit-report.md as a primary input — but do not limit yourself to it. Explore the codebase directly and independently to uncover additional improvement opportunities the audit may have missed: inspect UI components, menu structures, keyboard handling, file I/O, export pipelines, plugin architecture, preferences/config handling, and any other areas relevant to a desktop mind-mapping tool. Capture any further improvements you discover.

---

## Document Structure

The document must be organised by **Improvement**, not by generic category. Each improvement represents a specific, named problem or enhancement opportunity in the current application.

For each improvement, provide:

### Improvement [N]: [Specific, descriptive title — e.g. "Undo/Redo Stack Lost on Branch Move" or "No Keyboard Navigation in Node Formatting Panel"]

**Problem Statement:** A concise description of what is wrong or missing.

**How to Reproduce / Current State:** A short, concrete guide (3–8 steps) that tells a developer or tester exactly how to observe the current deficient behaviour in the running application. Be specific — name the menu, panel, keystroke, or action involved.

**Use Cases under this Improvement:** [list of UC numbers that address this improvement]

---

Then list each use case immediately after its parent improvement, using this structure:
...
| Scope & Level | System boundary and level (e.g., System/User-goal, Subfunction) |
...
Each improvement must have **at least 3 use cases**. The more meaningful use cases you can identify per improvement, the better. Use cases should cover distinct user goals, failure paths, edge cases, or actor variations — not paraphrase the same scenario.

---

## Quality Standards

- Every use case must represent an improvement, fix, or new capability — not existing functionality that already works.
- Use cases must be **high quality**: concrete, specific, and grounded in the actual application. Avoid vague generalisations.
- Fields like Preconditions and Triggers must reference real application states (e.g. "A mind map with at least one child node is open", "User presses Ctrl+Z").
- Aim for **30+ improvements** and **90+ use cases** total. Prioritise depth and real coverage over padding.
- Avoid grouping improvements by generic labels like "Security" or "UX". Each improvement title must describe the specific problem.
